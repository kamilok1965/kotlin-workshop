...
import coroutines.workshop.Decrypter
import coroutines.workshop.PasswordDecoded
import coroutines.workshop.PasswordPrepared
import java.util.*

class DecrypterImpl : Decrypter {
    companion object {
        private val maxClientCount = 4
    ...
    private val id = getNewId()

    override fun prepare(password: String): PasswordPrepared =
            PasswordPrepared(decrypt(id, password))

    override fun decode(state: PasswordPrepared): PasswordDecoded = PasswordDecoded(decrypt(id, state.password))

    override fun decrypt(state: PasswordDecoded): String = decrypt(id, state.password)
}